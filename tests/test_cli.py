...
class TestStdinPiping:
    ...
    def test_pipe_content_via_stdin(self):
        ...
        result, tmp = invoke(
            "add", "--title", "Piped Snippet", "--tag", "piped",
            input="def hello():\n    print('hello')\n",
        )
        assert result.exit_code == 0
        assert "Added snippet" in result.output
    ...
    def test_pipe_with_title_and_tags(self):
        ...
        result, tmp = invoke(
            "add", "--title", "Tagged Piped", "--tag", "python", "--tag", "cli",
            input="import sys\nprint(sys.version)\n",
        )
        assert result.exit_code == 0
        assert "Added snippet" in result.output
    ...
    def test_pipe_multiline_content(self):
        ...
        multiline = "def factorial(n):\n    if n <= 1:\n        return 1\n    return n * factorial(n - 1)\n"
        result, tmp = invoke(
            "add", "--title", "Factorial", "--tag", "python", "--tag", "recursion",
            input=multiline,
        )
        assert result.exit_code == 0
        assert "Added snippet" in result.output
    ...
    def test_pipe_with_language_flag(self):
        ...
        result, tmp = invoke(
            "add", "--title", "JS Pipe", "--tag", "js", "--language", "javascript",
    ...
    def test_pipe_content_is_sanitized(self):
        ...
        result, tmp = invoke(
            "add", "--title", "Special Chars", "--tag", "test",
